[Setup]
AppName=ExecDos without CallBack 2
AppVersion=1.0
CreateAppDir=no
Uninstallable=no
...
OutputBaseFilename=ExecDos without CallBack 2
; for InnoSetup 6+:
WizardResizable=yes
WizardStyle=modern
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ProcessID := ExecDos('cmd.exe', '', '', SW_SHOWMINIMIZED, nil, False);
        FilenameLabel.Caption := Format(#9'"cmd.exe" ProcessID: %d, and waiting for it''s ExitCode...', [ProcessID]);
        Update;
        ExitCode := Wait4ProcessExitCode(ProcessID);
        FilenameLabel.Caption := Format(#9'ProcessID: %d closed, displaying results msg.', [ProcessID]);
        Update;
      end;
      MsgBox( Format('ProcessID: %d' + #13#10 + 'ExitCode: %d ($%x)', [ProcessID, ExitCode, ExitCode]), mbInformation, MB_OK );
    end;
end;
