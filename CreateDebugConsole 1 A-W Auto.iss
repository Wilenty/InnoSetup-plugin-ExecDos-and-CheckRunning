[Setup]
AppName=CreateDebugConsole 1 A-W Auto
AppVersion=1.0
CreateAppDir=no
Uninstallable=no
...
OutputBaseFilename=CreateDebugConsole 1 A-W Auto
; for InnoSetup 6+:
WizardResizable=yes
WizardStyle=modern
...
procedure CurStepChanged(CurStep: TSetupStep);
  begin
    ...
    begin
      ...
      SetConsoleTextColors(BackNavy, TextYellow);
      SetConsoleColors(OldColor, OldColor);
      SetConsoleBufferSize(125, 32000);
      ProcessExitCode := ExecDos('cmd.exe', '/c "tree /a %WinDir%"', '', SW_HIDE, nil, True);
    end;
