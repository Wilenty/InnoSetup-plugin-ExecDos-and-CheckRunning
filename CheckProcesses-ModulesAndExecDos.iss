...
Function IsModuleLoaded(ModuleNameOrFullPath: PAnsiChar): Boolean;
  external 'IsModuleLoaded@files:ExecDosAndCheckRunning.dll stdcall';

Function IsProcessRunning(FileNameOrFullPath: PAnsiChar): Boolean;
  external 'IsProcessRunning@files:ExecDosAndCheckRunning.dll stdcall';
...
Function GetPIDpath(const PID: DWORD): PAnsiChar;
  external 'GetPIDpath@files:ExecDosAndCheckRunning.dll stdcall';
...
Function ProcessGetInfo(FileNameOrFullPath: PAnsiChar; out PID: DWORD): PAnsiChar;
  external 'ProcessGetInfo@files:ExecDosAndCheckRunning.dll stdcall';

Function ModuleGetInfo(ModuleNameOrFullPath: PAnsiChar; out IsLoaded: DWORD): PAnsiChar;
  external 'ModuleGetInfo@files:ExecDosAndCheckRunning.dll stdcall';
...
Procedure RichEditViewerWrite(RichEditViewer: TRichEditViewer; Str: String);
  begin
    RichEditViewer.Lines.Add(Str);
end;

procedure FileListCallBack(DosProgram, Params, Output: PAnsiChar);
  begin
    RichEditViewerWrite(FileListPage.RichEditViewer, Output);
end;

procedure TreeListCallBack(DosProgram, Params, Output: PAnsiChar);
  begin
    RichEditViewerWrite(TreeListPage.RichEditViewer, Output);
end;

procedure ErrorListCallBack(DosProgram, Params, Output: PAnsiChar);
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Add('This sample installer process ID is: ' + IntToStr(PID) + #13#10 + 'and it''s path is: "' + GetPIDpath(PID) + '"'#13#10);

          ProcessOrDLL := 'cmd.exe';
          If IsProcessRunning( ProcessOrDLL ) then
            Add('"' + ProcessOrDLL + '" is running...' + #13#10 + 'it''s path: "' + ProcessGetInfo(ProcessOrDLL, PID) + '"'#13#10'and process ID is: "' + IntToStr(PID) + '"'#13#10)
          else
            Add('"' + ProcessOrDLL + '" process not found.' + #13#10);

          ProcessOrDLL := 'Kernel32.dll';
          If IsModuleLoaded( ProcessOrDLL ) then
            Add('"' + ProcessOrDLL + '" is loaded...' + #13#10 + 'and it''s path is: "' + ModuleGetInfo(ProcessOrDLL, PID) + '"'#13#10)
          else
          ...
          Add('"' + ProcessOrDLL + '" is running...' + #13#10 + 'it''s path is: "' + ProcessGetInfo(ProcessOrDLL, PID) + '"'#13#10'process ID is: ' + IntToStr(PID) + #13#10'and first active window handle is: $' + Format('%x', [GetPIDhWnd(PID)]) + #13#10 );
...
Function Execute(const Prog, Params: String; ListPage: TOutputMsgMemoWizardPage; ExecDosCallBack: TExecDosCallBack): Integer;
  Var
    Int: Integer;
    Str: String;
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Update;
          Repaint;
          Result := ExecDos(Prog, Params, GetTempDir(), SW_HIDE, ExecDosCallBack);
          For Int := 0 to 30 do
            Str := Str + '-=';
          Str := Str + '-';
          Add(Str);
...
Function ExecuteWithParams(const CmdLine: String; ListPage: TOutputMsgMemoWizardPage; ExecDosCallBack: TExecDosCallBack): Integer;
  var
    Run: String;
    Int: Integer;
  begin
