...
OutputBaseFilename=Example Unicode
; for InnoSetup 6+:
WizardResizable=yes
WizardStyle=modern
...
// explanation of use -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> AfterInstall: ExecDosWithOutputOnInstallingPageA2W('program', 'cmd-line params', 'WorkDir', 'StatusMsg message', 'FilesMsg message'); // Working Dir and both last two messages can be empty
DestDir: "{tmp}"; Source: "1.cmd"; Flags: DeleteAfterInstall IgnoreVersion; Components: "execute\a"; AfterInstall: "ExecDosWithOutputOnInstallingPageA2W('""cmd.exe""', '/c ""{tmp}\1.cmd""', '', '{cm:StatusMsg}', '{cm:FilesMsg,{tmp}\1.cmd}')";
DestDir: "{tmp}"; Source: "2.cmd"; Flags: DeleteAfterInstall IgnoreVersion; Components: "execute\b"; AfterInstall: "ExecDosWithOutputOnInstallingPageA2W('""cmd.exe""', '/c ""{tmp}\2.cmd""', '', '{cm:StatusMsg}', '{cm:FilesMsg,{tmp}\2.cmd}')";
DestDir: "{tmp}"; Source: "3.cmd"; Flags: DeleteAfterInstall IgnoreVersion; Components: "execute\c"; AfterInstall: "ExecDosWithOutputOnInstallingPageA2W('""cmd.exe""', '/c ""{tmp}\3.cmd""', '', '{cm:StatusMsg}', '{cm:FilesMsg,{tmp}\3.cmd}')";
DestDir: "{tmp}"; Source: "4.cmd"; Flags: DeleteAfterInstall IgnoreVersion; Components: "execute\d"; AfterInstall: "ExecDosWithOutputOnInstallingPageA2W('""cmd.exe""', '/c ""{tmp}\4.cmd""', '', '{cm:StatusMsg}', '{cm:FilesMsg,{tmp}\4.cmd}')";
...
procedure InstallingCallBackW(const Output: WideString);
  begin
...
Procedure ExecDosWithOutputOnInstallingPageA2W(const FileNameOrFullPath, Params, WorkDir, StatusMsg, FilesMsg: String);
  Var
  ...
  begin
    ...
    begin
      ExitCode := ExecDosA2W( ExpandScript(FileNameOrFullPath), ExpandScript(Params), ExpandScript(WorkDir), SW_HIDE, @InstallingCallBackW, True );
      For Int := 0 to 30 do
        Str := Str + '-=';
      Str := Str + '-';
      Insert(0, Str);
...
procedure FileListCallBackW(const Output: WideString);
  begin
    MemoWrite(FileListPage.RichEditViewer, Output);
end;

procedure TreeListCallBackW(const Output: WideString);
  begin
    MemoWrite(TreeListPage.RichEditViewer, Output);
end;

procedure ErrorListCallBackW(const Output: WideString);
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Add('This sample installer process ID is: ' + IntToStr(ProcessID) + #13#10 + 'and it''s path is: "' + GetPIDpathW(ProcessID) + '"'#13#10);

          ProgramOrDLL := 'cmd.exe';
          If IsProgramRunningW( ProgramOrDLL ) then
            Add('"' + ProgramOrDLL + '" is running...' + #13#10 + 'it''s path is: "' + GetProgramInfoW(ProgramOrDLL, ProcessID) + '"'#13#10'and process ID is: ' + IntToStr(ProcessID) + #13#10)
          else
            Add('"' + ProgramOrDLL + '" process not found.' + #13#10);

          ProgramOrDLL := 'Kernel32.dll';
          If IsModuleLoadedW( ProgramOrDLL ) then
            Add('"' + ProgramOrDLL + '" is loaded...' + #13#10 + 'and it''s path is: "' + GetModuleInfoW(ProgramOrDLL, ProcessID) + '"'#13#10)
          else
          ...
          Add('"' + ProgramOrDLL + '" is running...' + #13#10 + 'it''s path is: "' + GetProgramInfoW(ProgramOrDLL, ProcessID) + '"'#13#10'process ID is: ' + IntToStr(ProcessID) + #13#10'and first active window handle is: $' + Format('%x', [GetPIDhWND(ProcessID)]) + #13#10 );
...
Function ExecuteW(const Prog, Params: WideString; ListPage: TOutputMsgMemoWizardPage; ExecOutCallBack: TExecOutCallBackW): Integer;
  Var
    Int: Integer;
    Str: String;
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Update;
          Repaint;
          Result := ExecDosW(Prog, Params, '', SW_HIDE, ExecOutCallBack, True);
          For Int := 0 to 30 do
            Str := Str + '-=';
          Str := Str + '-';
          Add(Str);
...
Function ExecuteWithParamsW(const CmdLine: WideString; ListPage: TOutputMsgMemoWizardPage; ExecOutCallBack: TExecOutCallBackW): Integer;
  var
    Run: WideString;
    Int: Integer;
  begin
    ...
    Int := Pos('" ', CmdLine);
    Run := Copy( CmdLine, 1, Int+1 );
    CmdLine := Trim( Copy( CmdLine, Int+1, Length(CmdLine) ) );
    Result := ExecuteW(Run, CmdLine, ListPage, ExecOutCallBack);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ExecuteWithParamsW(FileListPage.SubCaptionLabel.Caption, FileListPage, @FileListCallBackW);
      end
      ...
      begin
        ...
        ExecuteWithParamsW(ErrorListPage.SubCaptionLabel.Caption, ErrorListPage, @ErrorListCallBackW);
      end
      ...
      begin
        ...
        ExecuteWithParamsW(TreeListPage.SubCaptionLabel.Caption, TreeListPage, @TreeListCallBackW);
      end
