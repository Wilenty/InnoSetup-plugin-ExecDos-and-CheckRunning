[Setup]
AppName=ExecDos with InputCallBack Unicode
AppVersion=1.0
CreateAppDir=no
Uninstallable=no
...
OutputBaseFilename=ExecDos with InputCallBack Unicode
; for InnoSetup 6+:
WizardResizable=yes
WizardStyle=modern
...
FilesMsg=	Executing "%1" via "InputCallBackW"

[Files]
// explanation of use -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> -=> AfterInstall: WriteInputCallBackW('cmd-line params', 'StatusMsg message', 'FilesMsg message'); // both last two messages can be empty
DestDir: "{tmp}"; Source: "1.cmd"; Flags: DeleteAfterInstall IgnoreVersion; Components: "execute\a"; AfterInstall: "WriteInputCallBackW('""{tmp}\1.cmd""', '{cm:StatusMsg}', '{cm:FilesMsg,{tmp}\1.cmd}')";
DestDir: "{tmp}"; Source: "2.cmd"; Flags: DeleteAfterInstall IgnoreVersion; Components: "execute\b"; AfterInstall: "WriteInputCallBackW('""{tmp}\2.cmd""', '{cm:StatusMsg}', '{cm:FilesMsg,{tmp}\2.cmd}')";
DestDir: "{tmp}"; Source: "3.cmd"; Flags: DeleteAfterInstall IgnoreVersion; Components: "execute\c"; AfterInstall: "WriteInputCallBackW('""{tmp}\3.cmd""', '{cm:StatusMsg}', '{cm:FilesMsg,{tmp}\3.cmd}')";
DestDir: "{tmp}"; Source: "4.cmd"; Flags: DeleteAfterInstall IgnoreVersion; Components: "execute\d"; AfterInstall: "WriteInputCallBackW('""{tmp}\4.cmd""', '{cm:StatusMsg}', '{cm:FilesMsg,{tmp}\4.cmd}')";
...
procedure InstallingCallBackW(const OutputW: WideString);
  begin
...
Function EnumWindowsProcW(hwnd: HWND; strParamW: String): BOOL;
  Var
    WindowTitle: String;
    Int: Integer;
  begin
...
Procedure WriteInputCallBackW(Params, StatusMsg, FilesMsg: String);
  Var
    ParamsStr: String;
  begin
    With WizardForm do
    ...
    If DosWriteInputW( ParamsStr ) then
      Repeat
        AppProcessMessagesW();
        Sleep(50);
...
procedure CurStepChanged(CurStep: TSetupStep);
  begin
    ...
    begin
      EnumWindowsProcCallBack := CreateCallback(@EnumWindowsProcW);
      ProcessID := ExecDosW('cmd.exe', '/k echo off&cls', '', SW_SHOWMINNOACTIVE, @InstallingCallBackW, False);
    ...
    begin
      WriteInputCallBackW( 'exit /b %ErrorLevel%', 'Exiting the console program...', #9#9'bye, bye, see you next time. ;-)' );
